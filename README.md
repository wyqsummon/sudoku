...
- **数字锁定**：开启后点数字键即锁定该数字，全盘它的位置与候选数一起高亮；此时点格子就是填这个数字。
  某数字填满 9 个后自动跳到下一个没填完的数字；**已经是正确数字的格子点了不会有反应**（不会覆盖、不记错误）
...
设置里的「**画链用弧线**」（默认开）把链画成略带弧度的曲线，绕开直线路径上的候选数——
直线横跨好几格时会从沿途格子的候选数上压过去；同格内的短链仍画直线，关掉则一律画直线。
